...
void main() {
    ...
    if (resolutionLevelRand >= (1.0 - probabilityForMaxResolution)) {
        ...
        float maxMosaicSubGrid = 39.0; // モザイク時の最大subGrid (1から39段階)
        ...
        float subGridSize = floor(normalizedRand * maxMosaicSubGrid) + 1.0;
        subGridSize = clamp(subGridSize, 1.0, maxMosaicSubGrid);
