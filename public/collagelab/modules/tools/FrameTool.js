...
export function handleTransformPress() {
  if (!State.showFrames) return false;
  ...
  const additive = !!window.keyIsDown(window.SHIFT || 16);
  ...
  if (State.frameTransformAll) {
    ...
    // ハンドル/枠に当たってない場合は他ツールへ（点編集など）
    return false;
  }

  // 先に「既存選択のハンドル」を優先（ハンドルはポリゴン外にあるので hitFace 判定に依存しない）
  const preFaces = getSelectionFaces();
  if (preFaces.length > 0) {
    const sPre = ensureTransformSessionForFaces(preFaces);
    if (startDragIfHit(sPre)) return true;
  }

  // まずクリック対象で selection を更新
  const hitFace = findTopmostFaceAt(mx, my);
  if (hitFace) {
    // selection にいない face をクリックしたら単一選択（Shiftで追加）
    if (!Array.isArray(State.selectedFaces) || State.selectedFaces.length === 0) {
      setFaceSelection([hitFace], { additive });
    } else if (!State.selectedFaces.includes(hitFace)) {
      setFaceSelection([hitFace], { additive });
    } else {
      // selection 内クリックは primary だけ更新
      State.selectedFace = hitFace;
    }
  }

  const faces = getSelectionFaces();

  // 空打ち＋ドラッグで矩形選択
  if (!hitFace) {
    const s = ensureTransformSessionForFaces(faces.length ? faces : [/* empty */]);
    // selection が空でも select session は開始できるように作る
    State.frameTransform = State.frameTransform || {
      faces: [],
      baseByFace: [],
      baseCenter: { x: mx, y: my },
      baseW: 1,
      baseH: 1,
      sx: 1,
      sy: 1,
    ...
    t.dragging = { type: 'select', startX: mx, startY: my };
    t.selectRect = { x1: mx, y1: my, x2: mx, y2: my };
    t.selectAdditive = additive;
    t.selectBaseFaces = additive ? (State.selectedFaces || []).slice() : [];
    State.needsCompositeUpdate = true;
    return true;
  }
  ...
  const s = ensureTransformSessionForFaces(faces);
  if (!s) return false;
  if (startDragIfHit(s)) return true;

  // box 外を押したら矩形選択を開始
  s.dragging = { type: 'select', startX: mx, startY: my };
  s.selectRect = { x1: mx, y1: my, x2: mx, y2: my };
  s.selectAdditive = additive;
  s.selectBaseFaces = additive ? (State.selectedFaces || []).slice() : [];
  State.needsCompositeUpdate = true;
  return true;
}
...
export function handleFramePress() {
  ...
  if (State.isImagePlacing && State.placingImg) {
    ...
    if (target) {
      target.image = State.placingImg;
      target.imageName = State.placingImgName;
      State.selectedFace = target;
      State.needsCompositeUpdate = true;

      // 要望: 1回置いても配置モードを維持（連続で置ける）
  ...
  // フレームドラッグモード
  if (State.showFrames && !State.isPlacing) {
    const target = findTopmostFaceAt(window.mouseX, window.mouseY);
    if (target) {
